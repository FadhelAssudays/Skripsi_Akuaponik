...
int extractMinuteFromGSMTime(String timeStr) {
  int commaIndex = timeStr.indexOf(',');
  String minuteStr = timeStr.substring(commaIndex + 4, commaIndex + 6); // Ambil MM
...
void loop() {
  ...
  int menit = extractMinuteFromGSMTime(timee);
  if (jam >= 8 && jam < 16) {
    int sisa = menit % 30;

    if (sisa < 10) {
      pompa(125);  // Pompa ON 10 menit
    } else {
      pompa(0);    // Pompa OFF 20 menit
    }
  } else {
    pompa(0);      // Di luar jam kerja
  }
